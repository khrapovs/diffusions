...
def try_simulation() -> None:
    ...
    theta_true = CIRparam(mean=mean, kappa=kappa, eta=eta)
    ...
    nobs = nperiods * nsub
    paths = cir.simulate(start=x0, nsub=nsub, ndiscr=ndiscr, nobs=nobs, nsim=nsim)
    data = paths[:, 0, 0]

    plot_trajectories(paths=data, nsub=nsub, names="rates")
...
def try_marginal() -> None:
    ...
    plot_final_distr(paths=data, names="rates")
