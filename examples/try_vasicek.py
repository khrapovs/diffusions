...
def try_simulation() -> None:
    ...
    theta_true = VasicekParam(mean=mean, kappa=kappa, eta=eta)
    vasicek = Vasicek(theta_true)

    x0, nperiods, nsub, ndiscr, nsim = 1, 500, 2, 10, 3
    nobs = nperiods * nsub
    paths = vasicek.simulate(start=x0, nsub=nsub, ndiscr=ndiscr, nobs=nobs, nsim=nsim)
    data = paths[:, 0, 0]

    plot_trajectories(paths=data, nsub=nsub, names="returns")
...
def try_marginal() -> None:
    ...
    plot_final_distr(paths=data, names="returns")
