...
class RealizedMomentsHestonTestCase(ut.TestCase):
    ...
    def test_heston_var_instr(self) -> None:
        ...
        mom, dmom = heston.integrated_mom(
            theta, instr_data=instr_data, instr_choice="var", data=data, instrlag=instrlag
        )
    ...
    def test_const_instr(self) -> None:
        ...
        mom, dmom = heston.integrated_mom(theta, instr_choice="const", data=data, instrlag=instrlag)
        nmoms_all = nmoms
        mom_shape = (nperiods - instrlag, nmoms_all)
        ...
        error = depvar.dot(heston.mat_a(param, None).T) - heston.realized_const(param, aggh, None)
    ...
    def test_vol_p(self) -> None:
        ...
        mom, dmom = heston.integrated_mom(theta, subset=subset, instr_choice="const", data=data, instrlag=instrlag)
        nmoms = 2
        ...
        depvar = np.tile(depvar, 3)
        subset_sl = slice(2)
        error = depvar.dot(heston.mat_a(param, subset_sl).T) - heston.realized_const(param, aggh, subset_sl)
    ...
    def test_heston_relized_mom(self) -> None:
        ...
        depvar = np.ones((nperiods - instrlag, 4)) * means
        depvar = np.tile(depvar, 3)
        error_q = depvar.dot(heston.mat_a(param, subset_sl).T) - heston.realized_const(param, aggh, subset_sl)
        ...
        depvar = np.ones((nperiods - instrlag, 4)) * means
        depvar = np.tile(depvar, 3)
        error_p = depvar.dot(heston.mat_a(param, subset_sl).T) - heston.realized_const(param, aggh, subset_sl)
